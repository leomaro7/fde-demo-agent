...
初回は 40 秒ほど。既にある場合は変更がなければ数秒で終わり、出力だけ再表示される。
**2 周目でも壊れない。**
...
`infra/bin/app.ts` の案件一覧に import して足す。
...
`seed` と `tools.ts` はフロント側なので、そちらは 5 章だけでよい。
...
| | 時間 |
|---|---|
| 案件（Harness を含む） | 約 64 秒 |
| 土台（User Pool・Amplify・IAM） | 約 34 秒 |
...
| 3.3 デモユーザー | 実行。**2 周目**で `UsernameExistsException` が出ること、
  パスワード設定とグループ追加は何度でも通ることを確認 |
| 4 フロントを配信 | 実行。ジョブが `SUCCEED` になるまで確認。ローカル開発も起動を確認 |
| 5 動作を確認 | **実行。ブラウザで 3 問を通した。** 1・2 問目は根拠つきで回答、3 問目は拒否 |
| 6 撤去 | 実行。**残骸ゼロを確認** |

**初回構築（何も無い状態）からの通しは、この手順書では未検証。**
2 周目の状態から検証したため。土台 40 秒・案件 30 秒という初回の実測値は
2026-08-09 に別途取っている。
