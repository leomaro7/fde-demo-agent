...
**外向き・課金が発生する章（1.6 / 2 / 3.2 / 4）はエージェントでも流せる。**
ただし作る前に承認を取ること。
...
```
1.6 gh repo create ...             企業リポジトリを作る（新しいクライアントのときだけ）
...
### 1.6.1 土台リポジトリを template にする（初回だけ）
...
**消すのは自分の案件を作った後**（3 章の後）。先に消すと、`tsconfig.json` の
...
| 1.6 企業リポジトリを作る | **2026-08-14 に実行**（`fde-demo-verify0814` を作って通した）。下記 |
...
### 2026-08-14: 1.6 を実行して、書き直した

**未確認のまま残っていた唯一の章。** `fde-demo-verify0814` を実際に作って通した。
**3 か所が間違っていた。**

| 何が違ったか | 実際 |
|---|---|
| **template にする手順が丸ごと無い** | 土台は `isTemplate: false` だった。**そのままでは `--template` が失敗する**。1.6.1 として足した |
| 「`gh repo create` は `deny` に入れてある」 | **`ask` だった。** 承認を取れば流せる。「人でないと駄目」からも外した |
| 「見本を消す」を作成直後に置いていた | **先に消すと `npx tsc` が通らない。** `tsconfig.json` の `#demo` が指す先が消えるため。案件を作った後に回した |

**設計書 2026-08-13 が残していた未確認 2 件は、これで両方潰れた。**
`.claude/` と `docs/` はテンプレートからコピーされ、履歴は `Initial commit` の 1 つだけだった。

**土台側も直した。** 見本を消すと**土台の検査ごと落ちた**。

- `infra/lib/demo-stack.test.ts` が `demos/smoke` を import していた → 検査用の案件を書き下ろす形に変えた
- `demos/index.test.ts` が案件名を直書きしていた → 登録表から引く形に変えた

企業リポジトリを作るたびに同じ修正をさせるのは無駄なので、**土台で直した**。
見本ゼロの状態で `vitest` 130 件が通り、案件を 1 件置いて `tsconfig` を向け直すと
`tsc` も通り 142 件になることを、クローン側で確認した。
...
**対話入力の経路は 2026-08-14 に人が端末から通した。** エージェントからは流せない
（`--generate-password` を足したのはこれが理由）。
