...
`systemPrompt` には必ず 4 点を含める（要件書 4.2）。

1. 調べる順序 — どのツールをどの順で呼ぶか
2. 根拠の示し方 — 条番号、事例番号、計算に使った数字
3. **答えてはいけない条件** — 該当時に何と言い、どこに回すかまで
4. **探すのをやめる条件** — 検索して見つからないときに何回で諦め、何と答えるか

**4 は 2026-08-12 に足した。** 3 を書いてあっても、4 が無いとエージェントは
語を変えて検索し続け、ツール呼び出しの上限に達する。**商談の山場である
「答えを拒む場面」に到達せず、システムの中断メッセージで終わる。**
